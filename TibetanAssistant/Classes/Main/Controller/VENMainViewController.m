...
- (void)viewDidAppear:(BOOL)animated {
    ...
    if (!self.isRefresh) {
        ...
        if ([[VENClassEmptyManager sharedManager] isEmptyString:alert]) {
            ...
            if (!self.popupView) {
                VENPopupView *popupView = [[VENPopupView alloc] initWithFrame:self.view.bounds];
                [self.view addSubview:popupView];
                [popupView.button addTarget:self action:@selector(buttonClick1:) forControlEvents:UIControlEventTouchUpInside];
    ...
    tabBar.blk = ^(NSString *str) {
        ...
        if ([str isEqualToString:@"sjgx"]) {
            ...
            if (!self.popupView) {
                VENPopupView *popupView = [[VENPopupView alloc] initWithFrame:self.view.bounds];
                [self.view addSubview:popupView];
                [popupView.button addTarget:self action:@selector(buttonClick:) forControlEvents:UIControlEventTouchUpInside];
